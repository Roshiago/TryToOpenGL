#ifndef _FREE_TYPE_HEADER_
#define _FREE_TYPE_HEADER_
...
namespace FreeType
{
	...
	static void print(font_data& ft_font, float x, float y, const char *fmt, ...)
	{
		...
		if (fmt == NULL)            // Если это не текст
			*text = 0;                  // Тогда ничего не делать
			...
			vsprintf_s(text, fmt, ap);  // И конвертировать символы в числа
		...
		if (start_line) {
			string line;
			for (const char *n = start_line; *n; n++) 
				line.append(1, *n);
			lines.push_back(line);
		}
		...
		for (size_t i = 0; i<lines.size(); i++) 
		{
			...
			ft_font.w = rpos[0] - x + ft_font.h/1.5f; //(Надеюсь, что нет вращения)
			glPopMatrix();
		}
	...
	static void make_dlist(FT_Face face, char ch, GLuint list_base, GLuint * tex_base) 
	{
		...
		size_t width = next_p2(bitmap.width);
		size_t height = next_p2(bitmap.rows);
		...
		glBegin(GL_QUADS);
		glTexCoord2d(0.0, 0.0); glVertex2f(0.0, (float)bitmap.rows);
		glTexCoord2d(0.0, y); glVertex2f(0.0, 0.0);
		glTexCoord2d(x, y); glVertex2f((float)bitmap.width, 0.0);
		glTexCoord2d(x, 0.0); glVertex2f((float)bitmap.width, (float)bitmap.rows);
		glEnd();
		glPopMatrix();
		...
		glBitmap(0,0,0,0, (float)(face->glyph->advance.x >> 6),0.0,NULL);
		// Завершим создание списка отображения
		glEndList();
	}
}

#endif // !_FREE_TYPE_HEADER_
